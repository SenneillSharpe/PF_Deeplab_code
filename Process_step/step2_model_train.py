...
def model_train(
        train_model,
        train_epoch_num,
        train_device,  # 训练设备CPU或GPU
        train_model_save_base_path,  # 保存模型的路径
        train_log_save_path,
        train_dataset,
        valid_dataset,
        train_criterion
    ...
    train_dataloader = DataLoader(
        ...
        pin_memory=True,  # 图稿GPU数据传输效率
        drop_last=False  # 是否丢弃最后一批不足的样本，batch_size?
    )
    ...
    for epoch in range(1, train_epoch_num + 1):
        ...
        if (epoch % 50) == 0:
            torch.save(model, train_model_save_base_path + f"/model_save_{epoch}.pth")  # 保存当前状态下的模型及其参数
            model_params = model.state_dict()
            torch.save(model_params, train_model_save_base_path + f"/model_{epoch}.pth")


if __name__ == "__main__":
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    train_dataset = ThreeTrainDataset(
        image_dir="../Data2/train_image",
        segmentation_label_dir="../Data2/train_label",
    )
    valid_dataset = ThreeNonTrainDataset(
    ...
    model_train(
        ...
        train_epoch_num=100,
        train_device=device,
        train_model_save_base_path="../Model_save",
        train_log_save_path="record_log.txt",
        train_criterion=WeightedFocalLoss(),
